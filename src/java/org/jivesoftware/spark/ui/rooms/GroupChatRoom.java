...
public class GroupChatRoom extends ChatRoom {
	...
	private OleFrame infomationFrame;
	...
	public GroupChatRoom(final MultiUserChat chat) {
		...
		sendScreenShotButton = UIComponentRegistry.getButtonFactory()
				.createScreenshotButton();
		sendScreenShotButton.setToolTipText(Res
				.getString("message.send.picture"));
		this.addChatRoomButton(sendScreenShotButton);
		sendScreenShotButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource() == sendScreenShotButton) {
					...
					sendScreenshot(sendScreenShotButton, getChatRoom());
				} else {
					showFilePicker();
				}
			}
		...
		infomationFrame =new OleFrame(shell, SWT.NONE);
		OleControlSite clientsite = null;
        OleAutomation browser = null;
        try {
            clientsite = new OleControlSite(infomationFrame, SWT.NONE, "Shell.Explorer");
	...
	public void sendScreenshot(final ChatRoomButton button, final ChatRoom room) {
		...
		final ChatFrame chatFrame = SparkManager.getChatManager()
				.getChatContainer().getChatFrame();
			...
			public void finished() {
				...
				selectionPanel.addMouseListener(new MouseAdapter() {
					public void mouseReleased(MouseEvent e) {
						...
						try {
							newImage = bufferedImage.getSubimage(
									(int) clip.getX(), (int) clip.getY(),
									(int) clip.getWidth(),
									(int) clip.getHeight());
						} catch (Exception e1) {
							// Nothing to do
				...
				frame.setSize(bufferedImage.getWidth(null),
	...
	private void showFilePicker() {
		SwingWorker worker = new SwingWorker() {
			...
			public void finished() {
				FileDialog fileChooser = SparkManager.getTransferManager()
						.getFileChooser(
								SparkManager.getChatManager()
										.getChatContainer().getChatFrame(),
								Res.getString("title.select.file.to.send"));
				...
				File file = new File(fileChooser.getDirectory(),
						fileChooser.getFile());

				if (file.exists()) {
					SparkManager.getTransferManager().setDefaultDirectory(
							file.getParentFile());
					// SparkManager.getTransferManager().sendFile(file,
					// ((ChatRoomImpl)chatRoom).getParticipantJID());
				}
	...
	public void sendMessage(final BufferedImage image) {
		...
		SwingWorker writeImageThread = new SwingWorker() {
			...
			public void finished() {
				...
				if (imageFile.exists()) {
					try {
						getTranscriptWindow().insertIcon(new ImageIcon(imageFile.getAbsolutePath()));
						imageByte = image2String(imageFile);// 得到转码后的字符串
						getTranscriptWindow().insertText("\n");
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					sb.append("Tmp://" + imageFile.getName());
					sb.append("#");
					sb.append("<img>");
					sb.append(imageByte);
					sb.append("</img>");
	...
	public void sendMessage(Message message) {
		...
		try {
			//TODO check if is image send
			if (message.getBody().contains("Tmp://")
					&& message.getBody().contains("<img>")
					&& message.getBody().contains("</img>")) {
	...
	private void handleMessagePacket(Packet packet) {
		...
		if (message.getType() == Message.Type.groupchat) {
			...
			boolean isFromMe = messageNickname.equals(getNickname())
					&& inf == null;
			// Occupant sender =
			// SparkManager.getUserManager().getOccupant(this,message.getFrom());
			boolean isBlockMesage = BlockMesage(messageNickname);
			...
			if (ModelUtil.hasLength(message.getBody()) && !isFromMe) {
				...
				if (inf != null && !isBlockMesage &&!(message.getBody().contains("Tmp://")
						&& message.getBody().contains("<img>")
						&& message.getBody().contains("</img>"))) {
					...
					if (message.getBody().contains("Tmp://")
							&& message.getBody().contains("<img>")
							&& message.getBody().contains("</img>")) {
						Log.error("parse img code entered");
						File f= null;
						f= saveImageFromMessage(message);						
						message.setBody("image:");
						getTranscriptWindow().insertIcon(new ImageIcon(f.getAbsolutePath()));
						try {
							getTranscriptWindow().insertText("\n");
			...
			try {
				...
				if (message.getBody() != null) {
					// Create new room
					chatRoom = new ChatRoomImpl(message.getFrom(),
							userNickname, roomTitle);
	...
	private Boolean BlockMesage(String sender) {
		if (SparkManager.getUserManager().isOwnerOrAdmin(this, sender)
				|| (SparkManager.getUserManager().isOwnerOrAdmin(this,
						getNickname()))) {
			return false;
		} else
			return true;
	}
	private File saveImageFromMessage(Message message){
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (textFound.startsWith("Tmp://")) {
				String tmpPath = textFound.substring(
						textFound.indexOf("Tmp://") + 6,
						textFound.indexOf("#"));
				Log.error("screen shot file " + tmpPath
						+ "created.");
				// CHECK IF SEND BY ME, JUST INSERT EXISTED ICON
				// IF TRUE
				File rootPath = new File(
						Spark.getSparkUserHome(), "/tempImages");
				f = new File(rootPath.getAbsolutePath(),
						tmpPath);
				if (!f.exists()) {
					try {
						f.createNewFile();
			...
			if (textFound.contains("<img>")
					&& textFound.contains("</img>")) {
	 ...
	 * @TODO :justin is user isBlocked in our case user Blocked is modified
	 *       condition 1:if a message sender is an normal user all message is
	 *       blocked condition 2:user is owerner ,only block blockedusers
	 *       message
	 */
	...
	public void reconnectionSuccessful() {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			@Override
			public void run() {
				ConferenceUtils.joinConferenceOnSeperateThread(roomDesc,
						roomJID, password);
				closeChatRoom();
			}
		});
	...
	private void addRoomConfigureButtons(GroupChatRoom room) {
		...
		RolloverButton settings = UIComponentRegistry.getButtonFactory()
				.createSettingsButton();
		RolloverButton thema = UIComponentRegistry.getButtonFactory()
				.createTemaButton();
		RolloverButton register = UIComponentRegistry.getButtonFactory()
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ModelUtil.hasLength(newSubject)) {
					try {
						...
						getTranscriptWindow().insertNotificationMessage(
								xmpee.getMessage(), ChatManager.ERROR_COLOR);
						scrollToBottom();
					}
				}
			}
